...
| 			  		 | 	... instantiating a new agent?                      | Branch                  | Creator: Branch manages RealEstateAgent instances, so it should be responsible for creating them.                                |
| 			  		 | ... choosing a store for the agent to work from      | Branch                  | IE: The Branch class holds information about its associated agents and stores, making it suitable for managing store selections. |
| Step 2  		 | 	... requesting agent information from the user?				 | AgentView               | IE: This view is responsible for gathering the agent's details directly from the user interface.                                 |
| Step 3  		 | 	... saving the inputted data?                       | Branch                  | IE: object created in step 1 has its own data.                                                                                   | 
...
| Step 4  		 | 	... showing all data and request confirmation?      | RegisterAgentView       | IE: is responsible for user interactions.                                                                                        |
| Step 5  		 | 	... saving the new Agent?	                          | Branch                  | IE: Branch knows and manages its agents, so it should handle saving the new agent.                                               |
| Step 6  		 | 	... informing operation success?                    | RegisterAgentView       | Pure Fabrication: This view class is responsible for displaying operation results back to the user.                              | 
